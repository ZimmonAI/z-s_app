...
The 2B-06 workflow uses Node.js 22 and disposable PostgreSQL 17. It runs focused provider/media tests, registry integration tests, the full suite, typecheck, lint, build, clean package install, package artifact verification, migration/seed no-change validation, secret and legacy-identifier checks, the deterministic provider harness, and the complete validation chain.

Packaging and testing do not apply migrations, publish a package, access live providers, read credentials, deploy services, or modify consumers.
