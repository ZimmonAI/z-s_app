...
function parseAccountFile(text) {
  ...
  if (!clientId || !clientCredential) throw new Error('test-account-file-invalid');
...
async function runSmoke() {
  ...
  try {
    ...
    const suffix = new Date().toISOString().replace(/[-:TZ.]/g, '').slice(0, 14).toLowerCase();
    ...
    if (!positiveGate) throw new Error('positive-derivative-gate-failed');
    result.passed = true;
  } catch (error) {
