...
CREATE TABLE public.object_read_grants (
  ...
  caller_service_id text NULL CHECK (
    caller_service_id IS NULL OR
    (char_length(caller_service_id) BETWEEN 1 AND 96 AND caller_service_id ~ '^[a-z0-9][a-z0-9_-]*$')
  ),
  app_correlation_ref text NOT NULL CHECK (
    char_length(app_correlation_ref) BETWEEN 1 AND 128 AND
    app_correlation_ref ~ '^[A-Za-z0-9][A-Za-z0-9._:-]*$'
  ),
  business_authorization_ref text NOT NULL CHECK (
    char_length(business_authorization_ref) BETWEEN 1 AND 256 AND
    business_authorization_ref !~ E'[\r\n]'
  ),
  purpose text NOT NULL CHECK (
    char_length(purpose) BETWEEN 1 AND 128 AND
    purpose ~ '^[A-Za-z0-9][A-Za-z0-9._:-]*$'
  ),
  ...
  safe_file_name text NULL CHECK (
    ...
    (
      ...
      safe_file_name !~ E'[\u0000-\u001F\u007F/\\\\]' AND
      safe_file_name NOT IN ('.', '..')
    )
  ),
...
CREATE INDEX object_read_grants_object_expiry_idx
  ON public.object_read_grants (storage_object_id, expires_at);
...
COMMENT ON TABLE public.object_read_grants IS 'Durable short-lived server-mediated object read grants with digest-only token persistence. Ref: z-kn/08-execution/zimspace-storage-server-dev/tasks/in-progress/storage-platform-development/02b-07-package-read-grant-delivery-fallback-and-range.md';
