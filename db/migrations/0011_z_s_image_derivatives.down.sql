...
BEGIN
  IF to_regclass('public.storage_image_derivative_jobs') IS NULL
    ...
    RAISE EXCEPTION '0011 rollback unavailable: image derivative tables are missing';
  END IF;
  ...
  IF row_total <> 0 THEN
    RAISE EXCEPTION '0011 rollback blocked: adopted image derivative rows exist';
  END IF;
END
$$;

DROP TRIGGER z_s_image_derivative_output_guard_trigger
  ON public.storage_image_derivative_outputs;
DROP FUNCTION public.z_s_image_derivative_output_guard();
DROP TRIGGER z_s_image_derivative_job_guard_trigger
