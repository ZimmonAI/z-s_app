...
test('one listener dispatches write and read namespaces to their mature runtimes', async () => {
  ...
  assert.equal(
    await (await runtime.handle(new Request('http://local/v1/object-write-intents', {
      method: 'POST',
    }))).text(),
    'write',
  );
  assert.equal(
    await (await runtime.handle(new Request('http://local/v1/object-read-grants', {
      method: 'POST',
    }))).text(),
    'read',
  );
  assert.equal(
    await (await runtime.handle(new Request(
      'http://local/v1/storage-objects/00000000-0000-4000-8000-000000000099/content',
    ))).text(),
    'read',
  );
...
test('environment credential resolution uses indirection and fails closed', async () => {
  ...
  const resolver: ReadyProviderCredentialResolver =
    createEnvironmentCredentialResolver(environment);
...
test('authority resolution binds the exact active Video Maker development profile', async () => {
  ...
  await assert.rejects(
    registry.resolveStorageProfile(
      { ...request, environment: 'stg' },
      caller,
    ),
    (error: unknown) =>
      error instanceof RuntimeCompositionError &&
      error.code === 'storage-authority-request-mismatch',
  );
});

test('authority resolution refuses missing exact bindings and capability evidence', async () => {
  const missingAuthority = new PostgresVideoMakerAuthorityRegistry({
    pool: fakePool({ authorityRows: [] }),
  });
  await assert.rejects(
    missingAuthority.load(),
    (error: unknown) =>
      error instanceof RuntimeCompositionError &&
      error.code === 'storage-authority-unavailable',
  );

  const missingRange = new PostgresVideoMakerAuthorityRegistry({
    pool: fakePool({
      capabilityRows: CAPABILITIES.filter((row) => row.capability !== 'range'),
    }),
  });
  await assert.rejects(
    missingRange.load(),
    (error: unknown) =>
      error instanceof RuntimeCompositionError &&
      error.code === 'provider-range-capability-not-ready',
  );
});
