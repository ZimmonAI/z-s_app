...
The 2B-06 provider harness uses injected deterministic provider doubles. It verifies a fully successful upload and a contained one-provider failure without reading secrets, connecting to providers, or mutating a database.

The dedicated GitHub Actions workflow runs Node.js 22 with a disposable PostgreSQL 17 service, verifies that migrations and seeds are unchanged, and executes the complete validation chain. It does not connect to governed live infrastructure.
